...
# Hardcoded default W folder (last-resort only)
...
def _resolve_w_folder():
    ...
    # 2) local/script-relative fallbacks
    for candidate in [
        os.path.join(SCRIPT_DIR, 'w'),              # ./w next to this script
        os.path.join(os.getcwd(), 'w'),             # current working dir
        os.path.join(os.path.dirname(SCRIPT_DIR), 'w') # parent/w (repo root)
    ]:
        if os.path.isdir(candidate):
            return os.path.abspath(candidate)
    # 3) last resort hardcoded path
