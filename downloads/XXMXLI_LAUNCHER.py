        ...
        try:
            ...
            if script_path.endswith('.sh'):
                ...
                if platform.system().lower().startswith('win'):
                    # Try PowerShell if a corresponding .ps1 exists
                    ps1_candidate = os.path.splitext(script_path)[0] + '.ps1'
                    if os.path.isfile(ps1_candidate):
                        cmd = ['powershell', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', ps1_candidate]
                    else:
                        # Fallback to Git Bash or WSL if available
                        git_bash = os.path.join(os.environ.get('ProgramFiles', 'C:\\Program Files'), 'Git', 'bin', 'bash.exe')
